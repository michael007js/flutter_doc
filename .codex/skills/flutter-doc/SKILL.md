...
这个技能服务于 `C:\app\flutter doc` 中的 Flutter/Dart 文档本地化桌面工具项目。
...
1. 先读 [constraints](references/constraints.md)，确认不能突破的项目边界。
2. 再按任务类型读取对应主题文档，不要一次性加载全部 reference。
3. 需要交付或验收时，补读 [testing](references/testing.md) 和 [build](references/build.md)。
4. 遇到异常、历史坑或线程/数据库类问题时，补读 [errors](references/errors.md)。
